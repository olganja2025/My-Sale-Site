...
document.querySelectorAll('[data-anisen-contact]').forEach((root)=>{
  ...
  const termsHref=root.dataset.termsHref||'terms/';
  ...
  root.innerHTML=`<div class="messenger-contact-head"><h3>${contactTitle}</h3><p>Напишите удобным способом</p></div><div class="messenger-links">${links}</div><p class="messenger-legal">Продолжая, вы принимаете <a href="${termsHref}">Пользовательское соглашение</a> и даёте согласие на <a href="${consentHref}">обработку персональных данных</a>.</p>`;
});
